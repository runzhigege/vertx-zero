...
public class PackThread extends Thread {
    ...
    private static final Annal LOGGER = Annal.get(PackThread.class);

    private final transient String pkg;
    private final transient Predicate<Class<?>> filter;
    private final transient CountDownLatch count;
    ...
    @Override
    public void run() {
        final Set<Class<?>> clazzes = getClasses(this.filter, this.pkg);
    ...
    private Set<Class<?>> getClasses(final Predicate<Class<?>> filter,
    ...
    private Set<Class<?>> getClasses(final String packageDir,
