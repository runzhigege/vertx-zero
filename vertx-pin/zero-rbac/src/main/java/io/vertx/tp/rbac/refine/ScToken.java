...
class ScToken {
    ...
    static Future<Boolean> jwtToken(final List<OAccessToken> item, final String userId) {
        ...
        if (null == item) {
            ...
            if (item.isEmpty()) {
                ...
                final String tokenString = new String(token.getToken(), Values.CHARSET);
                final byte[] authBytes = userId.getBytes(Values.CHARSET);
    ...
    static OAccessToken jwtToken(final JsonObject jwt, final String userKey) {
        ...
        return new OAccessToken()
                ...
                .setAuth(userKey.getBytes(Values.CHARSET))
