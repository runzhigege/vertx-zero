...
public class UserService implements UserStub {
    ...
    /**
     *  TODO: default password should be generated by system and send it to user through email
     */
    private static final String DEFAULT_PASSWORD = "E559D4DA17DD1C17BE86FCF49E60E322";
    ...
    private Future<JsonObject> createOUser(final SUser user) {
        ...
        return Ux.Jooq.on(OUserDao.class)
                .insertAsync(oUser)
                .compose(entity -> Ux.fnJObject(user));
    }
    ...
    private Future<JsonObject> updateUserRelatedInformation(final String userKey, final JsonObject user, JsonArray roles, JsonArray groups) {
        List<String> rolesJObjects = roles.stream().map(item -> item.toString()).collect(Collectors.toList());
        List<String> groupsJObjects = groups.stream().map(item -> item.toString()).collect(Collectors.toList());
        ...
        return Ux.Jooq.on(RUserRoleDao.class)
            ...
            /* return user entity */
            .compose(userGroupList -> Ux.future(user.put("roles", roles).put("groups", groups)));
    }
}
