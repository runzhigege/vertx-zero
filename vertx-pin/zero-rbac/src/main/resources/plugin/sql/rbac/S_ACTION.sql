...
(
    ...
    `MODE`             VARCHAR(20) COMMENT '「mode」- 操作模式（决定是否触发副作用流程）, NODE | DEFINE',
    ...
    -- 副作用流程
    -- 1. 自定义副作用
    `EFFECT_COMPONENT` VARCHAR(255) COMMENT '「effectComponent」- mode = DEFINE，自定义副作用流程',
    -- 2. 副作用配置
    `EFFECT_CONFIG`    TEXT COMMENT '「effectConfig」- 副作用配置信息',
