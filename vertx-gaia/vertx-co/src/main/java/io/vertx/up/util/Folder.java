...
final class Folder {
    ...
    private static List<String> list(final String folder,
        ...
        if (folderObj.exists()) {
             ...
             */
            if (Objects.isNull(url)) {
                LOGGER.error("[ ZERO ] The url of folder = `{0}` is null", folder);
            } else {
                /*
                ...
                if (Protocols.FILE.equals(protocol)) {
                    ...
                    LOGGER.error("[ ZERO ] protocol error! protocol = {0}, url = {1}", protocol, url);
                }
            }
        }
        return retList;
    ...
    private static List<String> getFiles(final File directory, final String extension, final boolean isDirectory) {
        ...
        if (directory.isDirectory() && directory.exists()) {
            ...
            LOGGER.error("[ ZERO ] The file doest not exist, file = `{0}`", directory.getAbsolutePath());
        }
        return retList;
    }
